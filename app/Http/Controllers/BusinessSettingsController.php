...
class BusinessSettingsController extends Controller
{
    ...
    public function product_translate(Request $request)
    {
        $lang = $request->route('lang');
        if ($request->input('search')) {
            $lang ? $products = ProductTranslation::where('lang', $lang)->where('name', 'like', '%' . $request->input('search') . '%')->paginate(50) : $products = ProductTranslation::where('name', 'like', '%' . $request->input('search') . '%')->paginate(50);
        } else {
            $lang ? $products = ProductTranslation::where('lang', $lang)->paginate(50) : $products = ProductTranslation::paginate(50);
        }
        $page = 1;
        return view('backend.product.translate.index', compact('lang', 'page', 'products'));
    }


    public function translate_products_update(Request $request, ProductTranslation $product)
    {
    ...
    public function product_translate_copier(Request $request)
    {
        //Get product if not exist in frensh
        $products = ProductTranslation::where('lang', 'en')->get();

        foreach ($products as $product) {
            $product_fr = ProductTranslation::where('product_id', $product->product_id)->where('lang', 'fr')->first();
            if ($product_fr == null) {
                $product_fr = new ProductTranslation;
                $product_fr->product_id = $product->product_id;
                $product_fr->lang = 'fr';
                $product_fr->unit = $product->unit;
                $product_fr->name = $product->name;
                $product_fr->save();
            }
        }
        flash(translate('Product copy successfully'))->success();
        return back();
    }
}
