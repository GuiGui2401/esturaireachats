<form class="was-validated form-horizontal" action="{{ route('commissions.pay_to_seller') }}" method="POST"
    ...
    <div class="modal-body">
        ...
        @if ($user->shop->admin_to_pay > 0)
                ...
                <div class="col-sm-9">
                    @if ($seller_withdraw_request->amount > $user->shop->admin_to_pay)
                        <input type="number" lang="en" min="0" step="0.01" name="amount" id="amount"
                            value="{{ $user->shop->admin_to_pay }}" class="form-control" required>
                    @else
                        <input type="number" lang="en" min="0" step="0.01" name="amount" id="amount"
                            value="{{ $seller_withdraw_request->amount }}" class="form-control" required>
                    @endif
                </div>
            </div>
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-9">
                    <select name="payment_option" id="payment_option" class="form-control demo-select2-placeholder"
                        required>
