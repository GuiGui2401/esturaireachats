...
@section('content')
    ...
    <div class="card">
        <form class="was-validated " id="sort_products" action="" method="GET">
                ...
                <table class="table aiz-table mb-0">
                    ...
                    <tbody>
                        @foreach ($products as $key => $product)
                            <tr
                                class="{{ request()->has('productCheckingID') && in_array($product->id, explode(',', request()->input('productCheckingID'))) ? 'custom-bg-opacity' : '' }}">
                                @if (auth()->user()->can('product_delete'))
                                    <td>
                                ...
                                <td>
                                    <div class="row gutters-5 w-200px w-md-300px mw-100">
                                        <div class="col-auto">
                                            <img loading="lazy" src="{{ uploaded_asset($product->thumbnail_img) }}" alt="Image"
                                    ...
                                    @can('product_edit')
                                        @if ($type == 'Seller')
                                            <a class="btn btn-soft-primary btn-icon btn-circle btn-sm"
                                                href="{{ route('products.seller.edit', ['id' => $product->id, 'lang' => env('DEFAULT_LANGUAGE'), 'page' => request()->input('page', 1)]) }}"
                                                title="{{ translate('Edit') }}">
                                                <i class="las la-edit"></i>
                                            </a>
                                        @else
                                            <a class="btn btn-soft-primary btn-icon btn-circle btn-sm"
                                                href="{{ route('products.admin.edit', ['id' => $product->id, 'lang' => env('DEFAULT_LANGUAGE'), 'page' => request()->input('page', 1)]) }}"
                                                title="{{ translate('Edit') }}">
                                                <i class="las la-edit"></i>
                                            </a>
                                        @endif
...
@section('script')
    <script type="text/javascript">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const productCheckingID = new URLSearchParams(window.location.search).get('productCheckingID');
            if (productCheckingID) {
                const targetElement = document.querySelector('tr.custom-bg-opacity');
                ...
                if (targetElement) {
                    ...
                    console.log("aucun product check");
                }
            }
        });
    </script>
