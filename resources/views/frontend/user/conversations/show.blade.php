...
@section('panel_content')
    ...
    <div class="card rounded-0 shadow-none border">
        ...
        <div class="card-body">
            ...
            <!-- Conversations -->
            <div id="messages">
                @include('frontend.' . get_setting('homepage_select') . '.partials.messages', [
                    'conversation',
                    $conversation,
                ])
            </div>
            ...
            <!-- Send message -->
            <form class="was-validated pt-4" action="{{ route('messages.store') }}" method="POST">
                @csrf
                <input type="hidden" name="conversation_id" value="{{ $conversation->id }}">
                <div class="form-group">
                    <textarea class="form-control rounded-0" rows="4" name="message" placeholder="{{ translate('Type your reply') }}"
                        required></textarea>
                </div>
