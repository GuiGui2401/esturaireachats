 ...
 @section('content')
     <section class="container card border-1 rounded-2">
         <div class="aiz-editor-data">
             <img id="modalImage" src="{{ static_asset('assets/img/cards/master.png') }}" class="img-fit mb-2">
         </div>
         <div class="pb-5 pt-4 px-3 px-md-5">
             <h3>{{ translate('Make your payment securely! Fill in the form') }}</h3>
             <form action="{{ route('master.process.payment') }}" class="form-default was-validated" role="form"
                 method="POST">
                 @csrf
                 <input value="masterpay" class="online_payment" type="radio" id="payment_option" name="payment_option"
                     checked>
                 <div class="form-group mb-2">
                     <input type="email" class="form-control required-field" value="{{ old('reference', $reference) }}"
                         placeholder="Your email" name="reference" required>
                 </div>
                 <div class="form-group mb-2">
