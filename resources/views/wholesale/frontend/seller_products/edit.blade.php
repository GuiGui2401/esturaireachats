...
@section('panel_content')
    ...
    <div class="">
        <form class="was-validated form form-horizontal mar-top"
            action="{{ route('wholesale_product_update.seller', $product->id) }}" method="POST" enctype="multipart/form-data"
            id="choice_form">
                            ...
                            @foreach (\App\Models\Language::all() as $key => $language)
                                <li class="nav-item">
                                    <a class="nav-link text-reset @if ($language->code == $lang) active @else bg-soft-dark border-light border-left-0 @endif py-3"
                                        href="{{ route('wholesale_product_edit.seller', ['id' => $product->id, 'lang' => $language->code]) }}">
